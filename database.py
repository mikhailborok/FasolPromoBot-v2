    ...
    if promo_count == 0: # Добавляем акции только если их еще нет
        ...
        for store_id in range(1, 5):
            promotions_data.extend([
                (store_id, "☕ Кофе в подарок", "24.11.2025", 30, 100, 3, 1),  # 100 купонов, 3 дня на погашение, старт день в день
                (store_id, "📉 Скидка 5% на чек", "24.11.2025", 30, 0, 1, 0),   # Без лимита, 1 день, старт на следующий день
                (store_id, "🍭 Конфеты в подарок", "24.11.2025", 30, 50, 7, 1), # 50 купонов, 7 дней, старт день в день
            ])
